...
void CPODlg::ConvertImageToGrayscale()
{
	...
	start.x = 0;
	start.y = 0;
	end.x = m_imgIN.image.bitmapInfoHeader.biWidth;
	end.y = m_imgIN.image.bitmapInfoHeader.biHeight;

	CPaintDC dc(this);
	CRect sourceRect;
	sourceRect.SetRect(start, end);

	// setting resolution
	int horizontalPixels = GetDeviceCaps(dc.GetSafeHdc(), HORZRES);
	int verticalPixels = GetDeviceCaps(dc.GetSafeHdc(), HORZSIZE);
