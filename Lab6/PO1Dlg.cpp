...
BOOL CPODlg::OnInitDialog()
{
	...
	// lab5
	m_combo1.AddString(L"Pavlidis");
	m_combo1.AddString(L"DistanceMap");
	m_combo1.SelectString(0, L"Pavlidis");
...
void CPODlg::Dilatation()
{
	...
	int fileHeight = m_imgIN.imageFile.bitmapInfoHeader.biHeight;
	...
	start.x = 0;
	start.y = 0;
	end.x = m_imgIN.imageFile.bitmapInfoHeader.biWidth;
	end.y = m_imgIN.imageFile.bitmapInfoHeader.biHeight;
	rectangleIn.SetRect(start, end);
	...
	//	1	1	1	(x,y)		(x+1,y)		(x+2,y)	//  (x-1
	//  1	x   1	(x,y+1)   (x+1, y+1)	(x+2,y+1) // 
	//  1   1   1	(x,y+2)	(x+1, y+2)	(x+2,y+2) //

	//	(x-1,y-1)	(x,y-1) (x+1,y-1)	// p0	p1	p2
	//	(x-1,y)		(x,y)	(x+1,y)		// p3	p4	p5
	...
	if (m_imgIN.imageFile.bitmapInfoHeader.biBitCount == 1)
	{
		...
		for (int x = 0; x < fileWidth; x++) {
			for (int y = 0; y < fileHeight; y++) {
				...
				if (x - 1 < 0 || y - 1 < 0 || x + 1 >= end.x || y + 1 >= end.y) {
					m_imgOUT.imageFile.SetPixel8(x, y, 255);
					continue;
				}
				pixel = m_imgIN.imageFile.GetPixel1(x, y);

				if (!pixel) {
					m_imgOUT.imageFile.SetPixel8(x - 1, y - 1, 0);
					m_imgOUT.imageFile.SetPixel8(x, y - 1, 0);
					m_imgOUT.imageFile.SetPixel8(x + 1, y - 1, 0);
					m_imgOUT.imageFile.SetPixel8(x - 1, y, 0);
		...
		for (int x = 0; x < fileWidth; x++) {
			for (int y = 0; y < fileHeight; y++) {
				...
				if (!pixel) {
					...
					if (!m_imgIN.imageFile.GetPixel1(x - 1, y - 1) &&
						!m_imgIN.imageFile.GetPixel1(x, y - 1) &&
						!m_imgIN.imageFile.GetPixel1(x + 1, y - 1) &&
						!m_imgIN.imageFile.GetPixel1(x - 1, y) &&
...
void CPODlg::Opening()
{
	...
	//Pixel_map* v = new Pixel_map[size];

	bool* pixel_map = new bool[size];
	// Erozja 
	if (m_imgIN.imageFile.bitmapInfoHeader.biBitCount == 1)
	{
		for (int x = 1; x < fileWidth; x++) {
			for (int y = 1; y < fileHeight; y++) {
				...
				if (!pixel) {
					...
					if (!m_imgIN.imageFile.GetPixel1(x - 1, y - 1) &&
						...
						//m_imgOUT.imageFile.SetPixel8(x, y, 0);
						pixel_map[y * fileWidth + x] = 0;

					}
					else {
						//m_imgOUT.imageFile.SetPixel8(x, y, 255);
						pixel_map[y * fileWidth + x] = 255;
					}
				}
				else {
					//m_imgOUT.imageFile.SetPixel8(x, y, 255);
...
void CPODlg::KonturWew()
{
	...
	if (m_imgIN.imageFile.bitmapInfoHeader.biBitCount == 1)
	{
		for (int x = 0; x < fileWidth; x++) {
			for (int y = 0; y < fileHeight; y++) {
				if (x - 1 < 0 || y - 1 < 0 || x + 1 >= end.x || y + 1 >= end.y) {
					pixel_map[y * fileWidth + x] = m_imgIN.imageFile.GetPixel1(x, y);
					continue;
				}
	...
	if (m_imgIN.imageFile.bitmapInfoHeader.biBitCount == 1)
	{
		...
		for (int x = 0; x < fileWidth; x++) {
			for (int y = 0; y < fileHeight; y++) {
				...
				if (!pixel) {
					pixel_map[(y - 1) * fileWidth + x - 1] = 0;
					pixel_map[(y - 1) * fileWidth + x] = 0;
					pixel_map[(y - 1) * fileWidth + x + 1] = 0;
					pixel_map[y * fileWidth + x - 1] = 0;
					pixel_map[y * fileWidth + x] = 0;
					pixel_map[y * fileWidth + x + 1] = 0;
					pixel_map[(y + 1) * fileWidth + x - 1] = 0;
					pixel_map[(y + 1) * fileWidth + x] = 0;
		...
		for (int x = 1; x < fileWidth; x++) {
			for (int y = 1; y < fileHeight; y++) {
				...
				if (!pixel) {
					...
					if (!pixel_map[(y - 1) * fileWidth + x - 1] &&
						!pixel_map[(y - 1) * fileWidth + x] &&
						!pixel_map[(y - 1) * fileWidth + x + 1] &&
						!pixel_map[(y)*fileWidth + x - 1] &&
						!pixel_map[(y)*fileWidth + x] &&
						!pixel_map[(y)*fileWidth + x + 1] &&
						!pixel_map[(y + 1) * fileWidth + x - 1] &&
						!pixel_map[(y + 1) * fileWidth + x] &&
						!pixel_map[(y + 1) * fileWidth + x + 1]) {
...
void CPODlg::Pavlidis()
{
	...
	for (int y = 0; y < end.y; y++)
	{
		for (int x = 0; x < end.x; x++)
		{
			if (this->m_imgIN.image.GetPixel1(x, y) == true)
			{
				this->m_imgOUT.image.SetPixel8(x, y, BYTE(255));
			}
			else
			{
				this->m_imgOUT.image.SetPixel8(x, y, BYTE(0));
			}
			
		}
	}

	std::vector<int> vector2X;
	std::vector<int> vector2Y;

	std::vector<int> vector3X;
	std::vector<int> vector3Y;
		...
		for (int j = 1; j <= 7; j += 2)
		{
			...
			for (int y = 0; y < end.y; y++)
			{
				for (int x = 0; x < end.x; x++)
				{
					...
					if (this->m_imgOUT.image.GetPixel8(x, y) == BYTE(0) && this->m_imgOUT.image.GetPixel8(x + x1, y + y1) == BYTE(255))
					{
						while (true)
						{
							skel = false;
							...
							// 0 p 0
							// y y y
							bool xTop = m_imgOUT.image.GetPixel8(x - 1, y - 1) == BYTE(0) ||
								m_imgOUT.image.GetPixel8(x, y - 1) == BYTE(0) ||
								m_imgOUT.image.GetPixel8(x + 1, y - 1) == BYTE(0);

							bool maskA0 =
								m_imgOUT.image.GetPixel8(x - 1, y) == BYTE(255) &&
								m_imgOUT.image.GetPixel8(x + 1, y) == BYTE(255);
							...
							bool xRight = 
								m_imgOUT.image.GetPixel8(x + 1, y - 1) == BYTE(0) ||
								m_imgOUT.image.GetPixel8(x + 1, y) == BYTE(0) ||
								m_imgOUT.image.GetPixel8(x + 1, y + 1) == BYTE(0);
							...
							bool maskA90 =
								m_imgOUT.image.GetPixel8(x, y - 1) == BYTE(255) &&
								m_imgOUT.image.GetPixel8(x, y + 1) == BYTE(255);

							if (maskA90 && xRight && yLeft)
							{
								skel = true;
								break;
							}
						...
						if (skel)
						{
							...
							vector2X.push_back(x);
							vector2Y.push_back(y);
						}
						else
						{
							vector3X.push_back(x);
							vector3Y.push_back(y);
			...
			while (!vector3X.empty() && !vector3Y.empty())
			{
				int x = vector3X.back();
				int y = vector3Y.back();
				vector3X.pop_back();
			...
			/*for (int y = 0; y < end.y; y++)
			{
				for (int x = 0; x < end.x; x++)
				{
					if (this->m_imgOUT.image.GetPixel8(x, y) == BYTE(3))
					{
						this->m_imgOUT.image.SetPixel8(x, y, BYTE(255));
					}
				}
	...
	while (!vector2X.empty() && !vector2Y.empty())
	{
		int x = vector2X.back();
		int y = vector2Y.back();
		vector2X.pop_back();
