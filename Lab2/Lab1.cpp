...
bool Lab1::PaintDIB(HDC context, CRect destRect, CRect sourceRect)
{
	...
	{
		// update destWidth
		double proportion = (double)sourceWidth / (double)sourceHeight;
		sourceWidth *= (1 / proportion);
	}
	else {
		// update destHeight
		double proportion = (double)sourceHeight/ (double)sourceWidth;
		sourceHeight *= (1 / proportion);
	}
...
BYTE Lab1::ChangeBrightness(BYTE pixel, int range)
{
	if (this->bitmapInfoHeader.biBitCount != 8)
	{
		return 0;
	...
	int newPixel = pixel + range;

	newPixel = max(min(255, newPixel), 0);
		
	return newPixel;
}

BYTE Lab1::ChangeContrast(BYTE pixel, float multiplier)
{
	if (this->bitmapInfoHeader.biBitCount != 8)
	{
		return 0;
	}
	float offset = 127 * (1 - multiplier);

	int newPixel = (pixel*multiplier) + offset;
...
BYTE Lab1::ToPower(BYTE pixel,float exponent)
{
	if (this->bitmapInfoHeader.biBitCount != 8)
	{
		return 0;
	...
	float normalized = (float)pixel / 255;

	float newNormalized = pow(normalized,exponent);

	int newPixel = newNormalized * 255;
...
BYTE Lab1::Negative(BYTE pixel)
{
	...
	int difference = abs(pixel - 127);
	int newPixel = 0;

	if (pixel > 127)
	{
		newPixel = pixel - (2*difference);
	}
	else if (pixel < 127)
	{
		newPixel = pixel + (2 * difference);
